...
API reference собирается с помощью MkDocs и mkdocstrings. После установки dev-зависимостей:

```bash
mkdocs serve
```

Откройте в браузере адрес из вывода (обычно http://127.0.0.1:8000). Сборка статического сайта: `mkdocs build` (результат в `site/`).
