...
class PricesAPI(BaseAPI):
    ...
    def set_prices(self, data: list[dict]) -> Any:
        """Установка цен и скидок. Максимум 1000 товаров в запросе."""
        return self.post("/api/v2/upload/task", json={"data": data})

    def set_size_prices(self, data: list[dict]) -> Any:
        """Установка цен по размерам (для товаров с editableSizePrice)."""
        return self.post("/api/v2/upload/task/size", json={"data": data})

    def set_club_discounts(self, data: list[dict]) -> Any:
        """Установка скидок WB Клуба."""
        return self.post("/api/v2/upload/task/club-discount", json={"data": data})

    def get_processed_upload_state(self, upload_id: int) -> Any:
        """Статус обработанной выгрузки цен."""
        return self.get("/api/v2/history/tasks", params={"uploadID": upload_id})
    ...
    def get_unprocessed_upload_state(self, upload_id: int) -> Any:
        """Статус необработанной выгрузки."""
        return self.get("/api/v2/buffer/tasks", params={"uploadID": upload_id})
    ...
    def get_goods_with_prices(
        ...
        *,
        ...
        return self.get("/api/v2/list/goods/filter", params=params)

    def get_goods_with_prices_by_articles(self, nm_list: list[int]) -> Any:
        """Товары с ценами по артикулам (до 1000)."""
        return self.post("/api/v2/list/goods/filter", json={"nmList": nm_list})
